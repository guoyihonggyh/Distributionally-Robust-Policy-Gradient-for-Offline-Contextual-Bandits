...
from data import DATA, DATA_defined_prob, DATA_policy_gradient, DATA_fullaction, DATA_action, DATA_partial_logistic, \
    DATA_partial_logistic_deep, DATA_partial_random, DATA_partial_action, DATA_learn_policy, DATA_online
...
def train_regression(args, model, device, train_loader, optimizer, epoch, Myy, Myx, mean0, var0):
    ...
    for batch_idx, (data, target, weight) in enumerate(train_loader):
        ...
        output_last = ru.regression_gradient.apply(output, torch.tensor(Myx[0:-1]), torch.reshape(target, (bs, 1)),
                                                   torch.reshape(meanY, (bs, 1)))
...
def train_validate_test(args, epoch, loss_type, device, use_cuda, train_model, train_loader, test_loader,
                        validate_loader, n_class, lbd, testflag=True):
    if loss_type == 'regression':
        ...
        for epoch in range(1, epoch + 1):
            Myy, Myx = train_regression(args, train_model, device, train_loader, optimizer, epoch, Myy, Myx, mean0,
                                        var0)
...
def main():
    ...
    parser.add_argument('--epochs-training', type=int, default=30, metavar='N',
    ...
    parser.add_argument('--training-ratio', type=float, default=0.3, metavar='N', help='training-ratio')
    parser.add_argument('--online-ratio', type=float, default=0.3, metavar='N', help='online-ratio')
    parser.add_argument('--interaction-num', type=int, default=100, metavar='N', help='interaction-num')
    ...
    parser.add_argument('--interaction-epoch', type=int, default=1, metavar='N', help='interaction-epoch')
    parser.add_argument('--interaction-start-epoch', type=int, default=1, metavar='N', help='interaction-start-epoch')
    parser.add_argument('--ips-gradient', action='store_true', default=False)
    parser.add_argument('--ips-gradient-alpha', type=float, default=0.0, metavar='N', help='if use ips gradient')
    parser.add_argument('--epsilon', type=float, default=0.0, metavar='N', help='if use ips gradient')
    ...
    for simulation in range(args.simulation):
        ...
        if args.dataset != 4:
            ...
            online_train = int((online_ratio + training_ratio) * data_size)
            train_data = data.Subset(all_data, rand_idx[0: m_train])
            online_train_data = data.Subset(all_data, rand_idx[m_train: online_train])
            test_data = data.Subset(all_data, rand_idx[online_train: -1])
        ...
        test_loader = data.DataLoader(weighted_test, batch_size=1024, shuffle=False, **kwargs)
        ...
        initial_logging = prob_logging

        train_data_mse = DATA_defined_prob(train_data, n_class, n_dim, args, prob_logging)
        online_data = DATA_online(args, online_train_data, n_class, n_dim, train_data_mse)
        ...
        for epoch in range(1, args.epochs_policy_gradient + 1):
            if epoch > args.interaction_start_epoch:
                interaction_flag += 1
            ...
            if epoch == 1:
                ...
                train_data_robust = online_data


            for i in range(n_class):
                ## generate training set for action i
                train_action_data = DATA_partial_action(train_data_robust, i)
                test_policy_loader = data.DataLoader(train_action_data, batch_size=args.batch_size, shuffle=False, **kwargs)
                eval_policy = torch.empty([0, n_class])
                with torch.no_grad():
                    for feature, _ in test_policy_loader:
                        feature = feature.to(device)
                        output = policy_learning_model(feature)
                        prob = my_softmax(output)
                        eval_policy = np.concatenate((eval_policy, prob), axis=0)

                train_size = len(train_action_data)
                weight_st = prob_logging[:, i] / prob_action_pre[:, i]
                ...
                try:
                    ...
                    train_model, Myy, Myx, _, _, _ = train_validate_test(args, args.epochs_training, "regression", device,
                                                                         use_cuda, train_model,
            ...
            with torch.no_grad():
                for i in range(len(test_partial_data)):
                    features, action, reward, _ = test_partial_data[i]

                    action_policy = sample_action(prob_action_pre[i], n_class)
                    action_pre[i] = action_policy
                    policy = prob_logging[i, action_policy] / prob_action_pre[i][action_policy]
                    if policy > args.weights_upper_bound:
                        policy = torch.tensor([args.weights_upper_bound],dtype = torch.float64)
                    elif policy < args.weights_lower_bound:
                        policy = torch.tensor([args.weights_lower_bound],dtype = torch.float64)
                    output = model_robust_list[action_policy](features)

                    meanY_robust, varY = ru.predict_regression(torch.tensor(policy), Myy_robust_list[action_policy],
                                                               Myx_robust_list[action_policy], output, mean0, var0)
            ...
            for batch_idx, (feature_idx, action_idx, reward_idx, action_true_idx) in enumerate(train_loader):
                ...
                if online_data.ips_grad and args.ips_gradient:
                    output_learning_ips = policy_learning_model(torch.tensor(online_data.new_data))
                    prob_ips = my_softmax(output_learning_ips)

                    action_onehot_ips = torch.DoubleTensor(len(online_data.new_data), n_class)
                    action_onehot_ips.zero_()
                    action_onehot_ips.scatter_(1, torch.tensor(online_data.new_action,dtype=int).reshape(len(online_data.new_action), 1), 1)
                    ips_weight = prob_ips.detach().numpy()/online_data.add_logging_policy

                    ips_weight[np.isnan(ips_weight)] = 1
                    ips_weight[ips_weight<0.0001] = 0.0001
                    ips_weight[ips_weight>1000] = 1000

                    grad_policy = ru.policy_ips_gradient.apply(output_learning, prob, reward_idx, action_onehot, prob_ips, torch.tensor(online_data.new_reward), action_onehot_ips, torch.tensor(ips_weight),args.ips_gradient_alpha)
            ...
            prob_action_next = torch.empty([0, n_class])
            train_loader = data.DataLoader(policy_gradient_data,batch_size=args.batch_size, shuffle=False, **kwargs)
            for batch_idx, (feature_idx, action_idx, reward_idx, action_true_idx) in enumerate(train_loader):
                output_learning = policy_learning_model(feature_idx)
                prob = my_softmax(output_learning)
            ...
            if args.offline:
                interaction = online_data.interaction(policy_learning_model, initial_logging)
            else:
                interaction = online_data.interaction(policy_learning_model)
            if interaction:
                prob_logging = np.concatenate((prob_logging, online_data.add_logging_policy), axis=0)
                test_partial_data.add_training_data(online_data.new_data_add)
                prob_action_pre = torch.cat([prob_action_pre, torch.tensor(online_data.add_logging_policy)])
                action_pre = torch.ones(len(test_partial_data), dtype=torch.int64)

        print('Average loss: {:.4f}\n'.format(test_loss))

        savefilename = '' + args.save_file_name
        ...
        if args.dataset == 1:
            ...
            savefilename += 'ciafr10_'
        elif args.dataset == 4:
            savefilename += 'covertype_'
        else:
            savefilename += 'not_impletemnet'

        if args.policy == 0:
            savefilename += 'uniform_'
        elif args.policy == 1:
            savefilename += 'small_'
        elif args.policy == 2:
            savefilename += 'large_'
        elif args.policy == 3:
            savefilename += 'd_' + str(args.alpha) + '_'
        elif args.policy == 4:
            savefilename += 'tweak_' + str(args.rou) + '_'
        if args.epsilon>0:
            savefilename +=  'epsilon' + str(args.epsilon) + '_'


        if args.ips_gradient:
            savefilename += 'ipsg_'
            savefilename += str(args.ips_gradient_alpha)
        savefilename += str(simulation)
        savefilename += '_internum_'
        savefilename += str(args.interaction_num)

        np.savetxt('train_regret/' + savefilename + "_results_doubly_2.csv", np.array(regret_list), delimiter=",")
        np.savetxt('test_regret/' + savefilename + "_results_doubly_2.csv", test_regret_list, delimiter=",")
        np.savetxt('test_regret/' + savefilename + "_every_n_batch.csv", np.array(loss_evaluate_result), delimiter=",")
