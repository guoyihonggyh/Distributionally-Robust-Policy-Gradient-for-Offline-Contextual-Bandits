...
class policy_ips_gradient(torch.autograd.Function):
    '''
      back-prop gradients to network
      the gradient for "phi(x)" -- representing network -- is (y-mean_y)* Myx[0:-1]
    ...
    @staticmethod
    def forward(ctx, potential, prob, reward, action,prob_ips, reward_ips, action_ips,ips_weight,alpha):

        ctx.save_for_backward(potential, prob, reward, action,prob_ips, reward_ips, action_ips,ips_weight,torch.tensor(alpha))
        return potential

    @staticmethod
    def backward(ctx, grad_output):
        potential, prob, reward, action,prob_ips, reward_ips, action_ips,ips_weight,alpha = ctx.saved_tensors

        grad_x = grad_output.clone()

        if np.isnan(prob_ips).any():
            print(prob_ips,'prob_ips')
            print(prob_ips,'prob_ips',a)

        if np.isnan(action_ips).any():
            print(action_ips,'action_ips')
            print(action_ips,'action_ips',a)

        if np.isnan(ips_weight).any():
            print(ips_weight,'ips_weight')
            print(ips_weight,'ips_weight',a)

        if np.isnan(reward_ips).any():
            print(reward_ips,'reward_ips')
            print(reward_ips,'reward_ips',a)
        ...
        # print(torch.mean(torch.tensor(np.einsum('ij,i->ij', (prob_ips - action_ips) * ips_weight, reward_ips)),axis = 0))

        grad_x = grad_x * (torch.tensor(np.einsum('ij,i->ij', (prob - action), reward))*(1-alpha) + alpha * torch.mean(torch.tensor(np.einsum('ij,i->ij', (prob_ips - action_ips) * ips_weight, reward_ips)),axis = 0))
