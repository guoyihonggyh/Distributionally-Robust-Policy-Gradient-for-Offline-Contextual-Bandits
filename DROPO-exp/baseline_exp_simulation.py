...
from data import DATA, DATA_defined_prob, DATA_policy_gradient, DATA_fullaction, DATA_action, DATA_partial_logistic, DATA_partial_logistic_deep, DATA_partial_random, DATA_partial_action, DATA_learn_policy,DATA_online
...
def main():
    ...
    parser.add_argument('--lr-decay', type=float, default=0.5, metavar='N', help='dedcay rate of lr')
    ...
    parser.add_argument('--train-mse', type=bool, default=False, metavar='N', help='if use ips gradient')
    ...
    for simulation in range(args.simulation):
        ...
        # regret_list = np.zeros([args.epochs_policy_gradient,int(len(train_data)*10 + 10* len(online_train_data)/args.batch_size)+ 1])
        regret_list = []
        ...
        for epoch in range(1, args.epochs_policy_gradient + 1):
            ...
            if args.mode is 1 or args.mode is 3:
                ...
                Myx_robust_list = []
                # train_data_robust = train_data_mse
                # DATA_partial_logistic_deep(train_data, n_class,sample_model, estimated_policy = False)
                ...
                for i in range(n_class):
                    ...
                    try:
                        ...
                        if not args.train_mse:
                            ...
                            train_model = train_MSE(args, train_model, device, train_loader, optimizer1, args.epochs_training)
                    except:
                        Myy = np.ones((1, 1))
                        Myx = np.ones((d + 1, 1))
            ...
            if interaction:
                ...
                action_pre = torch.ones(len(online_data), dtype=torch.int64)
