...
def main():
    ...
    for simulation in range(args.simulation):
        ...
        for epoch in range(1, args.epochs_policy_gradient + 1):
            ...
            with torch.no_grad():
                for i in range(len(test_partial_data)):
                    ...
                    meanY_robust, varY = ru.predict_regression(torch.tensor(policy), Myy_robust_list[action_policy], Myx_robust_list[action_policy], output, mean0, var0)
        ...
        np.savetxt('train_regret/'+ args.save_file_name +dataset_name   + policy_name +  '_'  + str(simulation) +"results_dist_2.csv", np.array(regret_list), delimiter=",")
        np.savetxt('test_regret/'+ args.save_file_name +dataset_name   + policy_name  +  '_'  + str(simulation) +"results_dist_2.csv", test_regret_list, delimiter=",")
        np.savetxt('test_regret/'+ args.save_file_name +dataset_name   + policy_name  +  '_'  + str(simulation) +"every_n_batch.csv", np.array(loss_evaluate_result), delimiter=",")
